module "eks" {
  ...
  node_security_group_tags = {
    "karpenter.sh/discovery" = "${var.project_name}-prod-cluster"
  }

  eks_managed_node_groups = {
    app = {
      name = "${var.project_name}-prod-nodegroup"
      ami_type       = "BOTTLEROCKET_x86_64"  # <-- BOTTLEROCKET_ARM_64 or BOTTLEROCKET_x86_64
      instance_types = ["t3.medium"]
      iam_role_name  = "${var.project_name}-prod-nodegroup"
      ...
      launch_template_tags = {
        Name = "${var.project_name}-prod-node"
      }
