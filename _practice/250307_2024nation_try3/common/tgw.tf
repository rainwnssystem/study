locals {
  ...
  tgw_target_vpcs = [
    "vpc-0bf5ff87e87d14d77",
    "vpc-0490446bf367d18c2",
    "vpc-0b40ff3e2134bb20f",
  ]

  # Corresponding names for each TGW attachment
  # The index here must match the index of the VPC IDs above
  tgw_attachment_names = [
    "${var.project_name}-ma-tgw-attach",
    "${var.project_name}-prod-tgw-attach",
    "${var.project_name}-storage-tgw-attach"
  ]

  # Names for TGW route tables
  tgw_route_table_names = [
    "${var.project_name}-ma-tgw-rt",
    "${var.project_name}-prod-tgw-rt",
    "${var.project_name}-storage-tgw-rt"
  ]
}
